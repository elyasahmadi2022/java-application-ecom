package com.ecomerence.gateway.security;
...
//                        .pathMatchers("/api/v1/products/**").hasRole("PRODUCT")
//                        .pathMatchers("/api/v1/users/**").hasRole("USER")
//                        .pathMatchers("/api/v1/orders/**").hasRole("ORDER")
    ...
    public Converter<Jwt, Mono<AbstractAuthenticationToken>> grantedAuthoritiesExtractor(){
        ...
        reactiveJwtAuthenticationConverter.setJwtGrantedAuthoritiesConverter(jwt -> {
            List<String> roles = jwt.getClaimAsMap("resource_access")
                    .entrySet().stream()
                    .filter(entry -> entry.getKey().equals("ecom-app-client-pkce"))
